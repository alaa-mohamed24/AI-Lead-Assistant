...
st.set_page_config(
    page_title="Real Estate AI Lead Assistant",
    page_icon="🏢",
    layout="wide"
)
...
if "current_lead" not in st.session_state:
    st.session_state.current_analysis={"score":0,"calssification": "N/A" }  
...
with tab1:
    ...
    with col_chat:
        ...
        if user_input := st.chat_input("Type Your Message here...."):
            ...
            with st.spinner("AI is thinking & analyzing...."):
                ...
                st.session_state.conversation_history.append({"role": "user", "parts": [{"text": user_input}]})
                st.session_state.conversation_history.append({"role": "model", "parts": [{"text": ai_response}]})
                st.session_state.messages.append({"role": "assistant", "content": ai_response})
                
                # Update Extracted Data States
    ...
    with col_info:
        ...
        classification = str(analysis.get("classification", "N/A")).upper()
        ...
        if lead:
            ...
            st.markdown(f"**🏠 Property:** {lead_dict.get('property_type') or '---'}")
            st.markdown(f"**📍 Location:** {lead_dict.get('location') or '---'}")
            st.markdown(f"**💰 Budget:** {lead_dict.get('budget') or '---'}")
            st.markdown(f"**🛏️ Bedrooms:** {lead_dict.get('bedrooms') or '---'}")
            st.markdown(f"**🎨 Finishing:** {lead_dict.get('finishing') or '---'}")
            st.markdown(f"**⏱️ Timeline:** {lead_dict.get('timeline') or '---'}")
        else:
...
with tab2:
    ...
    # جلب البيانات من داتابيز SQLite
    ...
    if leads_data:
        ...
        m2.metric("Hot Leads 🔥", len(df[df['classification'].str.lower() == 'hot']))
        m3.metric("Warm Leads ⚡", len(df[df['classification'].str.lower() == 'warm']))
